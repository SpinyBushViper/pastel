local c = require('pastel.colors')
local cfg = vim.g.pastel_config
local util = require('pastel.util')
...
function M.setup()
    ...
    if color_name:sub(1, 1) == '$' then
      ...
      if not color_name then
        vim.schedule(function()
          vim.notify(
            'pastel.nvim: unknown color "' .. name .. '"',
  ...
  for group_name, group_settings in pairs(vim.g.pastel_config.highlights) do
